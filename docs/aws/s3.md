---
sidebar_position: 3
title: S3 - Armazenamento de Objetos
description: Armazenamento escalável e durável para qualquer tipo de arquivo
---

# 📦 S3 - Simple Storage Service

**S3** é o serviço de armazenamento de objetos da AWS. Pense nele como um "disco infinito na nuvem" para guardar qualquer arquivo: fotos, vídeos, documentos, backups, etc.

## 🤔 O que é armazenamento de objetos?

Diferente de um sistema de arquivos tradicional, S3 armazena **objetos** em **buckets**:

```
Sistema de Arquivos Tradicional:
/home/usuario/
├── /documentos/
│   ├── relatorio.pdf
│   └── projeto.docx
└── /imagens/
    ├── foto1.jpg
    └── foto2.png

S3 (Object Storage):
meu-bucket/
├── documentos/relatorio.pdf
├── documentos/projeto.docx
├── imagens/foto1.jpg
└── imagens/foto2.png

⚠️ Não são "pastas reais", é apenas chave (key)!
```

## 🎯 Características principais

### **Durabilidade 99.999999999%** (11 noves!)

```
De 1 milhão de arquivos, você perde 1 a cada 10.000 anos!
```

### **Escalabilidade ilimitada**

```
┌─────────────────────────────────┐
│ Você pode armazenar:            │
│ • 1 objeto de 5TB               │
│ • 1 bilhão de objetos           │
│ • Exabytes de dados             │
│ • Sem limite teórico            │
└─────────────────────────────────┘
```

### **Acesso via URL**

```
https://s3.amazonaws.com/meu-bucket/imagens/foto.jpg
```

## 💾 Estrutura básica

### **Bucket**

Contêiner para armazenar objetos. Precisa ter nome único globalmente:

```
Regras de nomes:
✅ meu-bucket-pessoal
✅ 2024-backups-empresa
❌ MeuBucket (MAIÚSCULA)
❌ meu_bucket (underscore)
❌ meu.bucket (ponto no início/fim)
```

### **Objeto**

Arquivo em si. Definido por:

- **Key**: O "caminho" do arquivo
- **Value**: Conteúdo do arquivo
- **Metadata**: Informações sobre o arquivo
- **Tags**: Rótulos para organização

## 🔒 Classes de armazenamento

S3 oferece diferentes classes para otimizar preço/performance:

### **S3 Standard** (Padrão)

Acesso frequente, alta performance:

```
Latência: Milissegundos
Preço: Mais caro/mês
Use para: Websites, aplicações, conteúdo ativo
```

### **S3 Standard-IA** (Infrequent Access)

Acesso infrequente, mas rápido quando necessário:

```
Preço: 50% mais barato que Standard
Taxa de recuperação: Pequena taxa por acesso
Use para: Backups mensais, logs antigos
```

### **S3 Intelligent-Tiering**

Mova automaticamente entre classes:

```
Dias 0-30:     S3 Standard (acesso ativo)
Dias 31-90:    S3 Standard-IA (acesso infrequente)
Dias 91+:      S3 Glacier (arquivo)

Quando acessar, volta para Standard!
```

### **S3 Glacier**

Arquivamento de longo prazo:

```
Latência: Horas ou dias
Preço: Muito barato
Use para: Arquivos legais, conformidade
```

## 💾 Exemplo de uso via AWS CLI

```bash
# Criar bucket
aws s3 mb s3://meu-bucket-pessoal

# Upload de arquivo
aws s3 cp documento.pdf s3://meu-bucket-pessoal/documentos/

# Download de arquivo
aws s3 cp s3://meu-bucket-pessoal/documentos/documento.pdf ./

# Listar objetos
aws s3 ls s3://meu-bucket-pessoal/

# Sincronizar pasta (tipo Dropbox)
aws s3 sync ./local-backup s3://meu-bucket-pessoal/backups/

# Deletar arquivo
aws s3 rm s3://meu-bucket-pessoal/documentos/documento.pdf
```

## 🌐 Hospedagem de Website Estático

S3 pode servir um website estático diretamente:

```
┌──────────────────────────┐
│ Seu site estático        │
├──────────────────────────┤
│ index.html               │
│ style.css                │
│ script.js                │
│ images/                  │
└──────────────────────────┘
         │
         └─→ Upload para S3
              │
         S3 habilita
         "Static Website"
              │
         https://meu-site.s3.amazonaws.com
```

## 🔐 Segurança e Controle de Acesso

### **Block Public Access**

Impede acesso público acidental:

```
┌─────────────────────────────┐
│ Block Public Access        │
│ ✓ Block all public ACLs    │
│ ✓ Ignore all ACLs          │
│ ✓ Block public bucket      │
│ ✓ Block public CORS        │
└─────────────────────────────┘
```

### **Versionamento**

Mantenha histórico de versões:

```
documento.txt
├── Versão 1: "Olá mundo"
├── Versão 2: "Olá mundo!" (com !)
└── Versão 3: "Hello world"

Pode restaurar qualquer versão anterior!
```

## 📊 Comparação de classes

| Classe           | Preço | Latência | Caso de uso       |
| ---------------- | ----- | -------- | ----------------- |
| **Standard**     | $$$   | até 1ms  | Aplicações ativas |
| **Standard-IA**  | $$    | até 1ms  | Backups mensais   |
| **Glacier**      | $     | Horas    | Arquivo legal     |
| **Deep Archive** | $     | 12h      | Raro acesso       |

## 💰 Preços aproximados (US)

```
Armazenamento:
├── Standard: $0.023/GB/mês
├── Standard-IA: $0.0125/GB/mês
└── Glacier: $0.004/GB/mês

Requisições:
├── PUT/GET: $0.0005 por 10.000 requisições
└── LIST: Baixa taxa

Transferência de dados:
├── Para fora da AWS: $0.09/GB
└── Dentro da AWS: Grátis!
```

## 🚀 Próximas lições

1. [EC2](/docs/aws/ec2) - Instâncias que leem/escrevem em S3
2. [CloudFront](/docs/aws/) - CDN para servir S3 mundialmente
3. [Load Balancers](/docs/aws/load-balancers/) - Distribuir tráfego
